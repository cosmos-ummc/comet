...
func (s *GetFeedsHandler) GetFeeds(ctx context.Context, req *pb.CommonGetsRequest) (*pb.CommonFeedsResponse, error) {
	...
	if len(req.Ids) > 0 {
		...
		feeds, err := s.Model.BatchGetFeeds(ctx, req.Ids)
		if err != nil {
	...
	total, feeds, err := s.Model.QueryFeeds(ctx, sort, itemsRange, filter)
	if err != nil {
...
func (s *GetFeedsHandler) processReq(ids []string) []string {
